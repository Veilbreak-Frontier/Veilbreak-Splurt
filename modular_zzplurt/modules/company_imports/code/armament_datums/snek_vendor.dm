/datum/armament_entry/company_import/snekoil
	category = COMPANY_NAME_SNAKE_OIL_BIOSOLUTIONS
	company_bitflag = CARGO_COMPANY_SNAKE_OIL_BIOSOLUTIONS

/datum/armament_entry/company_import/snekoil/nifsoft
	subcategory = "NIF Items ((More Coming Soon™))"

/datum/armament_entry/company_import/snekoil/nifsoft/sobnif
	item_type = /obj/item/organ/cyberimp/brain/nif/sobnif
	cost = PAYCHECK_COMMAND * 50

/datum/armament_entry/company_import/snekoil/nifsoft/money
	item_type = /obj/item/disk/nifsoft_uploader/money_sense
	cost = PAYCHECK_COMMAND * 15


/datum/armament_entry/company_import/snekoil/chips
	subcategory = "Skillchips"

/datum/armament_entry/company_import/snekoil/chips/xeno
	item_type = /obj/item/skillchip/xenoarch_magnifier
	cost = PAYCHECK_COMMAND * 15

/datum/armament_entry/company_import/snekoil/chips/robo
	item_type = /obj/item/skillchip/job/roboticist
	cost = PAYCHECK_COMMAND * 15

/datum/armament_entry/company_import/snekoil/chips/engi
	item_type = /obj/item/skillchip/job/engineer
	cost = PAYCHECK_COMMAND * 15

/datum/armament_entry/company_import/snekoil/chips/miner
	item_type = /obj/item/skillchip/job/miner
	cost = PAYCHECK_COMMAND * 15

/datum/armament_entry/company_import/snekoil/chips/chef
	item_type = /obj/item/skillchip/job/chef
	cost = PAYCHECK_COMMAND * 15

/datum/armament_entry/company_import/snekoil/chips/janitor
	item_type = /obj/item/skillchip/job/janitor
	cost = PAYCHECK_COMMAND * 15

/datum/armament_entry/company_import/snekoil/chips/organ
	item_type = /obj/item/skillchip/entrails_reader
	cost = PAYCHECK_COMMAND * 15

/datum/armament_entry/company_import/snekoil/cyber
	subcategory = "Augmentations ((More Soon™))"

/datum/armament_entry/company_import/snekoil/cyber/omni
	item_type = /obj/item/autosurgeon/combi
	cost = PAYCHECK_COMMAND * 20

/datum/armament_entry/company_import/snekoil/genetic
	subcategory = "Genetics"

/datum/armament_entry/company_import/snekoil/genetic/nose
	item_type = /obj/item/dnainjector/olfaction
	cost = PAYCHECK_COMMAND * 8

/datum/armament_entry/company_import/snekoil/genetic/web
	item_type = /obj/item/dnainjector/webbing
	cost = PAYCHECK_COMMAND * 8

/datum/armament_entry/company_import/snekoil/genetic/cry
	item_type = /obj/item/dnainjector/cryokinesis
	cost = PAYCHECK_COMMAND * 8

/datum/armament_entry/company_import/snekoil/medical
	subcategory = "Medical ((More Soon™))"

/datum/armament_entry/company_import/snekoil/medical/ifak
	item_type = /obj/item/storage/medkit/tactical
	cost = PAYCHECK_COMMAND * 10

/datum/armament_entry/company_import/snekoil/medical/grower
	item_type = /obj/item/circuitboard/machine/limbgrower
	cost = PAYCHECK_COMMAND * 10

/datum/armament_entry/company_import/snekoil/medical/botany
	item_type = /obj/item/circuitboard/machine/chem_dispenser/mutagensaltpeter
	cost = PAYCHECK_COMMAND * 10
