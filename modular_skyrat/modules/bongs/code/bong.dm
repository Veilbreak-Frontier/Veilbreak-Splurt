...
/obj/item/bong/proc/light(flavor_text = null)
	...
	if(reagents.get_reagent_amount(/datum/reagent/toxin/plasma)) // the plasma explodes when exposed to fire
		var/datum/effect_system/reagents_explosion/explosion = new(get_turf(src), round(reagents.get_reagent_amount(/datum/reagent/toxin/plasma) * 0.4, 1), 0, 0)
		explosion.start(src)
		qdel(src)
		return
	if(reagents.get_reagent_amount(/datum/reagent/fuel)) // the fuel explodes, too, but much less violently
		var/datum/effect_system/reagents_explosion/explosion = new(get_turf(src), round(reagents.get_reagent_amount(/datum/reagent/fuel) * 0.2, 1), 0, 0)
		explosion.start(src)
		qdel(src)
		return
