...
/datum/vote/transfer_vote/finalize_vote(winning_option)
    ...
    if(T && T.round_start_time)
        ...
        var/hours = floor(elapsed_time / 36000)
        var/minutes = floor((elapsed_time % 36000) / 600)
        var/seconds = floor((elapsed_time % 600) / 10)

        var/h_str = (hours < 10) ? "0[hours]" : "[hours]"
        var/m_str = (minutes < 10) ? "0[minutes]" : "[minutes]"
        var/s_str = (seconds < 10) ? "0[seconds]" : "[seconds]"

        duration_text = "[h_str]:[m_str]:[s_str]"
