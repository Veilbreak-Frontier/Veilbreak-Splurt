// Upgrading Elder Atmosian gear with voidshards.
// Modifies the items directly to apply the void effect.

/obj/item/clothing/suit/armor/elder_atmosian/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	if(istype(tool, /obj/item/voidshard))
		if(name == "Void-Infused [initial(name)]")
			to_chat(user, "<span class='warning'>\The [src] is already infused with the void!</span>")
			return ITEM_INTERACT_BLOCKING

		to_chat(user, "<span class='notice'>You crush \the [tool] against \the [src], infusing it with void energy!</span>")
		playsound(loc, 'modular_zzveilbreak/sound/effects/shard-infusion.mp3', 50, 1)

		name = "Void-Infused [initial(name)]"
		desc = "[initial(desc)] It pulses faintly with dark, purple energy."
		color = "#8a2be2" // Voidshard purple glow
		light_range = 2
		light_power = 0.5
		light_color = "#8a2be2"

		// Create a stronger armor datum for the void version based on the original
		set_armor(get_armor().generate_new_with_modifiers(list(ARMOR_ALL = 20)))
		attach_clothing_traits(TRAIT_RADIMMUNE)

		qdel(tool)
		return ITEM_INTERACT_SUCCESS
	return ..()

/obj/item/clothing/head/helmet/elder_atmosian/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	if(istype(tool, /obj/item/voidshard))
		if(name == "Void-Infused [initial(name)]")
			to_chat(user, "<span class='warning'>\The [src] is already infused with the void!</span>")
			return ITEM_INTERACT_BLOCKING

		to_chat(user, "<span class='notice'>You crush \the [tool] against \the [src], infusing it with void energy!</span>")
		playsound(loc, 'modular_zzveilbreak/sound/effects/shard-infusion.mp3', 50, 1)

		name = "Void-Infused [initial(name)]"
		desc = "[initial(desc)] It pulses faintly with dark, purple energy."
		color = "#8a2be2"
		light_range = 2
		light_power = 0.5
		light_color = "#8a2be2"

		set_armor(get_armor().generate_new_with_modifiers(list(ARMOR_ALL = 20)))
		attach_clothing_traits(TRAIT_RADIMMUNE)

		qdel(tool)
		return ITEM_INTERACT_SUCCESS
	return ..()

/obj/item/storage/backpack/hydro_duffel/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	if(istype(tool, /obj/item/voidshard))
		if(name == "Void-Infused [initial(name)]")
			to_chat(user, "<span class='warning'>\The [src] is already infused with the void!</span>")
			return ITEM_INTERACT_BLOCKING

		to_chat(user, "<span class='notice'>You crush \the [tool] against \the [src], infusing it with void energy!</span>")
		playsound(loc, 'modular_zzveilbreak/sound/effects/shard-infusion.mp3', 50, 1)

		name = "Void-Infused [initial(name)]"
		desc = "[initial(desc)] It pulses faintly with dark, purple energy, seeming bigger on the inside."
		color = "#8a2be2"
		light_range = 2
		light_power = 0.5
		light_color = "#8a2be2"

		if(atom_storage)
			atom_storage.max_slots += 10
			atom_storage.max_total_storage += 50
			atom_storage.max_specific_storage = WEIGHT_CLASS_GIGANTIC

		qdel(tool)
		return ITEM_INTERACT_SUCCESS
	return ..()
