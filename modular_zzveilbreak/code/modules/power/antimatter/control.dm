...
/obj/machinery/power/am_control_unit/process()
	...
	if(!fueljar)
		toggle_power()
		playsound(src.loc, 'sound/machines/buzz/buzz-two.ogg', 50, 0)
		return
...
/obj/machinery/power/am_control_unit/proc/produce_power()
	...
	var/core_power = reported_core_efficiency
	if(core_power <= 0)
		return 0
	var/core_damage = 0
...
/obj/machinery/power/am_control_unit/take_damage(damage, damage_type = BRUTE, sound_effect = 1)
	switch(damage_type)
		if(BRUTE)
			if(sound_effect)
				if(damage)
					playsound(loc, 'sound/items/weapons/smash.ogg', 50, 1)
				else
					playsound(loc, 'sound/items/weapons/tap.ogg', 50, 1)
		if(BURN)
			if(sound_effect)
				playsound(src.loc, 'sound/items/tools/welder.ogg', 100, 1)
		else
			return
	if(damage >= 20)
		stability -= damage/2
...
/obj/machinery/power/am_control_unit/proc/check_stability()
	if(stability <= 0)
		qdel(src)
	return
...
/obj/machinery/power/am_control_unit/proc/check_shield_icons()
	if(shield_icon_delay)
		return
	shield_icon_delay = 1
	if(update_shield_icons == 2)
		for(var/obj/machinery/am_shielding/AMS in linked_shielding)
			if(AMS.processing)
				AMS.shutdown_core()
			AMS.control_unit = null
